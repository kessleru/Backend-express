 ...
 *
 * A divisão é entre erro que NÓS criamos — cuja mensagem pode ir ao cliente — e
 * qualquer outra coisa, que é bug e vira 500 genérico. Mensagem de bug descreve
 * o banco ou a infraestrutura, e entregá-la é dar um mapa a quem estiver sondando.
 */
...
/** 422: o servidor entendeu a requisição inteira e recusa pelo conteúdo dela. */
export const dadosInvalidos = (detalhes: unknown) =>
  new AppError('Dados inválidos', 422, detalhes);

/**
 * 401 — "não sei quem você é". Vale para token ausente, malformado, com
 * assinatura errada ou expirado, e também para o login que não confere.
 */
 ...
 *
 * Um 403 devolvido no lugar de um 404 conta ao cliente que o recurso existe.
 * Quando decidir entre os dois for uma escolha de fato, ela é tomada no serviço,
 * não aqui: ver `servico.ts`.
 */
export const semPermissao = (mensagem: string) => new AppError(mensagem, 403);
