...
export function limitar(max: number, janelaMs: number) {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (atual.total > max) {
      ...
      //
      // Note que o header é posto ANTES do throw: `res` já foi tocado e o
      // tratador de erro só vai definir status e corpo. Header e corpo são
      // independentes até a resposta ser enviada.
      res.set('Retry-After', String(segundos));
      throw new AppError(`Muitas requisições. Tente novamente em ${segundos}s.`, 429);
    }
