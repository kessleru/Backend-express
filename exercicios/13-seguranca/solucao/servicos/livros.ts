...
export function criarServicoLivros(
  ...
  return {
    ...
    // `emprestar` e `devolver` SAÍRAM deste service no módulo 11.
    //
    // Eles viraram `servicos/emprestimos.ts`, porque a operação deixou de ser
    // "virar um booleano no livro" e passou a criar uma entidade com dono e
    // datas. Manter uma cópia aqui daria dois caminhos para a mesma regra — e um
    // deles (este) não saberia nada sobre quem pode devolver.
    //
    // Princípio: **uma regra, um dono.** Quando a responsabilidade muda de casa,
    // a versão antiga sai; ela não fica "por compatibilidade".
  };
}
