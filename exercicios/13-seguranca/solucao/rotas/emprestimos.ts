...
export function criarRotasEmprestimos(servico: ServicoEmprestimos): Router {
  ...
  /**
   * As duas rotas do módulo 13. Repare que elas levam só `autenticar`.
   *
   * Não tem `exigirPapel` porque a regra não é sobre papel — é "dono OU admin",
   * e depende de buscar o empréstimo. E não existe um middleware `exigirDono`:
   * ele teria que consultar o repositório, duplicando a busca que o service já
   * faz, e a regra passaria a valer só para quem entra pelo HTTP.
   *
   * A ordem importa: `/meus` (literal) foi declarado ANTES de `/:id`. Invertendo,
   * `GET /emprestimos/meus` cairia em `/:id` com `id = 'meus'`, e o `idSchema`
   * responderia 400 — um erro que parece do cliente e é de montagem (módulo 04).
   */
  router.get('/:id', autenticar, validar(idSchema, 'params'), controller.buscar);
  router.post(
    '/:id/devolver',
    autenticar,
    validar(idSchema, 'params'),
    controller.devolverPorEmprestimo,
  );
