...
import { criarApp, type App, type OpcoesApp } from '../app.ts';
...
export function montarApp(
  ...
  const app = criarApp(
    ...
    { rateLimit: false, ...opcoes },
  );
  return { app, repoLivros };
}

/**
 * O app com os limitadores LIGADOS, para os testes que provam o 429.
 *
 * Cada chamada devolve um app com baldes ZERADOS, porque `criarApp` chama
 * `criarLimites()` — os contadores nascem junto com o app, não com o módulo.
 *
 * É por isso que esta função não precisa "limpar" nada antes. Enquanto os
 * limitadores eram `export const`, ela precisava chamar `resetKey()` com o IP
 * que o Supertest usa (`::ffff:127.0.0.1`) — um detalhe interno da lib que
 * ninguém deveria ter que saber. Ver o bloco em `middlewares/limites.ts`.
 */
export function montarAppComLimite() {
  return montarApp(livrosDeTeste(), autoresDeTeste(), { rateLimit: true });
}
